...
gh_format_error() {
    ...
    if gh_is_network_error "$exit_code" "$output"; then
        if [ "$exit_code" = "124" ]; then
...
#
# Runs the command and sets GH_LAST_WAS_NETWORK_ERROR based on result.
...
# Sets: GH_LAST_WAS_NETWORK_ERROR
# shellcheck disable=SC2034  # GH_LAST_WAS_NETWORK_ERROR used by callers
